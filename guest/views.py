...
def approveLeave(request,id=0,leaveid=0):
    if request.method == "GET":
            ...
            leavereqModel.save()
            try:
                employeeLeave = EmployeeLeave.objects.get(employee_id=id,for_month_id=month)
...
def generatesalarymonth(request):
    if request.method == "POST":
        ...
        if employeeSalary.objects.filter(employee=employees,salary_month = monthObject).exists():
            print(employees)
            print(salaryMonth)
            ...
            bonuses = Bonus.objects.filter(employee=employee_id, bonus_month=salaryMonth,status=1)
            totalBonus = sum(bonus.bonus_amount for bonus in bonuses)
            reductions = Reduction.objects.filter(employee=employee_id, reduction_month=salaryMonth,status=1)
            totalReduction = sum(reduction.reduction_amount for reduction in reductions)
            try:
            ...
            for reduction in reductions:
                ...
                reduction.save()
            employeeleaveModel = EmployeeLeave.objects.get(employee=employees,for_month=monthObject)
            employeeleaveModel.sick_leaves_monthly = 0
            employeeleaveModel.casual_leaves_monthly = 0
            employeeleaveModel.half_day_leaves_monthly = 0
            employeeleaveModel.excess_leave_monthcl = 0
